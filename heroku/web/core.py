...
class Web(root.Web):
    ...
    async def start(self, port: int, proxy_pass: bool = False):
        ...
        # Handle Replit environment ports
        if any([os.environ.get("REPL_ID"), "replit" in os.environ.get("USER", "").lower()]):
            # For Replit Deployments, use PORT env var; for workspace, use 5000
            if os.environ.get("REPLIT_DEPLOYMENT"):
                self.port = int(os.environ.get("PORT", 5000))
            else:
                self.port = 5000
        elif any(env_var in os.environ for env_var in ["RENDER", "RENDER_SERVICE_NAME", "RENDER_EXTERNAL_URL"]):
            # Render.com environment - use PORT env var or default to 10000
            self.port = int(os.environ.get("PORT", 10000))
        else:
            self.port = os.environ.get("PORT", port)
        # Bind to 0.0.0.0 for cloud environment compatibility
        host = "0.0.0.0"
