...
# Глобальная переменная для управления сообщениями о соединении
_connection_spam_disabled = True

def override_text(exception: Exception) -> typing.Optional[str]:
    """Returns error-specific description if available, else `None`"""
    # Проверяем настройку для сообщений о проблемах с интернетом
    if isinstance(exception, (TelegramNetworkError, asyncio.exceptions.TimeoutError)):
        if _connection_spam_disabled:
            return None  # Не показываем сообщение если отключено в настройках
        else:
...
def init_connection_spam_setting():
    """Initialize connection spam setting from database"""
    global _connection_spam_disabled
    try:
        # Попытка загрузить настройку из базы данных
        from . import database
        db = database.Database()
        _connection_spam_disabled = db.get("LogSettingsMod", "connection_spam_disabled", True)
    except Exception:
        # Если база недоступна, используем значение по умолчанию
        _connection_spam_disabled = True

def init():
    # Инициализируем настройку соединения
    init_connection_spam_setting()
    ...
    class NoConnectionSpamFilter(logging.Filter):
        def filter(self, record: logging.LogRecord) -> bool:
            # Проверяем глобальную настройку
            if not _connection_spam_disabled:
                return True  # Показываем все сообщения если фильтр отключен
                
            msg = record.getMessage()
            # Блокируем сообщения о проблемах с соединением если фильтр включен
            connection_keywords = [
            ...
            return not any(keyword.lower() in msg.lower() for keyword in connection_keywords)
    
    logging.getLogger("aiogram.dispatcher").addFilter(NoFetchUpdatesFilter())
    logging.getLogger().addFilter(NoConnectionSpamFilter())
