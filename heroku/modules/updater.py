...
class UpdaterMod(loader.Module):
    ...
    def get_changelog(self) -> str:
        try:
            ...
            if not os.path.exists('.git'):
                logger.debug("No .git directory found, skipping changelog")
                return False
    ...
    def get_latest(self) -> str:
        try:
            if not os.path.exists('.git'):
                logger.debug("No .git directory found, cannot get latest commit")
                return ""
            ...
            logger.debug(f"Failed to get latest commit: {e}")
            return ""
